...
const EditJobGroup = ({ match }) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      const newJobgroup = {
        departmentno: input.jobgroupno,
        departmentname: input.jobgroupname,
        departmentdescription: input.jobgroupdescription,
      };
  ...
  useEffect(() => {
    jobgroup.map((data) =>
      fetch(`/jobgroups/${data.id}`)
        .then((res) => {
          if (res.ok) {
            return res.json();
          }
        })
        .then((jsonRes) => setJobGroup(jsonRes))
    );
  });

  //handle filtered Data
  function getPagedData() {
    const allJobgroups = jobgroup;
                  ...
                  <Form.Group className="mb-3">
                    <InputGroup>
                      ...
                      <FormControl
                        ...
                        as="textarea"
                        rows={3}
                        value={jobgroup.jobgroupdescription}
                        onChange={handleChange}
                      />
